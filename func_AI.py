...
def zeroth_energy(v, J, N):
     ...
     for k in range(1, N, 1):
         for i in range(k):
             sgn1 =  (-1)**(jnp.sum(jax.lax.dynamic_slice(v,[ind[i]],[ind[k]-ind[i]]) ))
             
             #sgn1 =  (-1)**(jnp.sum(v[ind[i]:(ind[k])]))
             v_intermediate = v.at[ind[i]].set(0)
             v_intermediate = v_intermediate.at[ind[k]].set(0)
             sgn2 =  (-1)**(jnp.sum(jax.lax.dynamic_slice(v_intermediate,[ind[i]],[ind[k]-ind[i]]) ))
             #sgn2 =  (-1)**(jnp.sum(v_intermediate[ind[i]:(ind[k])]))
...
@partial(jax.vmap, in_axes = (0, 0, None, None, None))
def energy_elements(v, trans, J, L, N):
      ...
      for i in range(first.shape[0]):
          H_syk = H_syk.at[first[i]].set(first_ord_energy(v, trans[first[i]], J, L, N))
...
def Exact_ground_gen(L, seed):
         ...
         seed_mat = energy_elements(states, trans, J, L, N)
