...
vs  = nk.vqs.MCState(nk.sampler.MetropolisExchange(hilbert =hi, graph = my_graph, d_max = L),model = model , n_samples = 1000)
sr = nk.optimizer.SR()
...
lr = [0.05, 0.01, 0.001, 0.0001, 0.00005]
ers = [0.01, 0.001, 0.0005, 0.0001, 0.00005]
for step in range(5):
    print("Actual Learning rate :", lr[step])
    gs.optimizer = nk.optimizer.Sgd(learning_rate = lr[step])
    for i in range(2000):
                  
                  gs.advance(steps = 50)
                  means.append(np.real(gs.energy.Mean))
                  variance.append(np.real(gs.energy.Variance))
                  new_e = np.real(gs.energy.Mean)
                  if abs((new_e - old_e)/new_e) < error:
                      print("Result converged up to error")
                      break
                  old_e = new_e
...
ID = "Adaptive_run_L_"+str(L)+"seed_"+"{:.2f}".format(seed)+"alpha_"+"{:.2f}".format(alpha)+"error_"+"{:.6f}".format(error)
print("time evaluation_sg", time.time() - t0)
np.save("time_evaluation_sg" + ID, time.time()-t0)
np.save("syk_mean__"+ID+".npy", means)
np.save("syk_variance__"+ID+".npy", variance)
np.save("syk_steps__"+ID+".npy", steps)
